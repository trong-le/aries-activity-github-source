...
export default class GithubSource extends Activity {
    ...
    async listIssues({ user, repo, owner, since, state = 'all' }, gh) {
    ...
    async listMergedPullRequests({ user, repo, owner, state = 'all' }, gh) {
        let mergedPRs = [];
    ...
    async listPullRequests({ user, repo, owner, state = 'all' }, gh) {
