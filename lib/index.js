...
export default class GithubSource extends Activity {
    ...
    async getAllUser(config, gh, owner) {
        const { user, method, appendRepo } = config;
        let promises = [];
        const repos = await this.listRepos({ user }, gh);
        for (const repo of repos) {
            if (repo.size !== 0) {
                const resp = await this[method]({ user, repo: repo.name, owner, appendRepo }, gh);
    ...
    async getAllOrg(config, gh) {
        const { user, org, method, owner, state, since, until, appendRepo } = config;
        const promises = [];
