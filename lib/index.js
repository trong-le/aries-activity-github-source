...
export default class GithubSource extends Activity {
    ...
    async listIssueComments({ user, repo, number, owner }, gh) {
        ...
        let comments = await getComments({ user, repo, number, owner });
    ...
    async listIssueEvents({ user, repo, number, owner }, gh) {
        ...
        let events = await getEventsTimeline({ user, repo, issue_number: number, owner });
    ...
    async listRepoEvents({ user, repo, owner }, gh) {
        ...
        let events = await getEventsForRepo({ user, repo, owner });
        let returner = events.data;
        while (gh.hasNextPage(events)) {
            events = await gh.getNextPage(events);
            returner = returner.concat(events.data);
    ...
    async listReposForUser({ user }, gh) {
        ...
        let repos = await getForUser({ username: user });
        let returner = repos.data;
        while (gh.hasNextPage(repos)) {
            repos = await gh.getNextPage(repos);
            returner = returner.concat(repos.data);
