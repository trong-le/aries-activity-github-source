...
export default class GithubSource extends Activity {
    ...
    async listIssues({ user, repo, owner, state = 'open'}, gh) {
        const getForRepo = promisify(gh.issues.getForRepo, gh);
        const issues = await getForRepo({ owner, repo, state });
    ...
    async listMergedPullRequests({ user, repo, owner, state = 'open' }, gh) {
        let mergedPRs = [];
    ...
    async listPullRequests({ user, repo, owner, state  = 'open'}, gh) {
    ...
    async getAllOrg(config, gh) {
        const { user, org, method, owner, state } = config;
        const promises = [];
        const repos = await this.listOrgRepos({ org }, gh);
        for (const repo of repos) {
            if (repo.size !== 0) {
                const resp = await this[method]({ user, repo: repo.name, owner, state}, gh);
