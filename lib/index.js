...
export default class GithubSource extends Activity {
    ...
    async onTask(activityTask, config, lastExecutionDate) {
        let data = null;
        ...
        if (config.since === 'lastRun'){
            this.since = lastExecutionDate;
        } else if (typeof config.since !== 'undefined'){
            this.since = config.since;
        } else {
            this.since = '2004-01-01T00:00:00.000Z';
        }

        if (config.until === 'lastRun'){
            this.until = lastExecutionDate;
        } else if (typeof config.until !== 'undefined'){
    ...
    async listAvailableAssignees({ user, repo, owner }, gh) {
    ...
    async listRepoComments({ user, repo, owner }, gh,lastExecutionDate) {
    ...
    async listRepoCommits({ user, repo, owner }, gh) {
        ...
        console.log("lastExecutionDate: " + JSON.stringify(this.lastExecutionDate));
        console.log("since: " + JSON.stringify(since));
