...
export default class GithubSource extends Activity {
    ...
    async listIssues({ user, repo, owner, since, state = 'open' }, gh) {
    ...
    async getAllOrg(config, gh) {
        const { user, org, method, owner, state, since, until } = config;
        const promises = [];
        const repos = await this.listOrgRepos({ org }, gh);
        for (const repo of repos) {
            if (repo.size !== 0) {
                const resp = await this[method]({ user, repo: repo.name, owner, state, since, until }, gh);
