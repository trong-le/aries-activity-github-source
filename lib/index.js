...
export default class GithubSource extends Activity {
    ...
    async listIssues({ user, repo }, gh) {
        this.log.debug('listIssues ' + repo);
    ...
    async getAll(config, gh) {
        return new Promise((resolve, reject) => {
            ...
            let data = [];
            this.listRepos({ user }, gh).then(repos => {
                this.log.debug(repos);
                repos.forEach((repo, index, array) => {
                    this[method]({ user, repo: repo.name }, gh).then(result => {
                        this.log.debug('result: ' + JSON.stringify(result));
                        if (result.length) data.push(result);
