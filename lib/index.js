...
export default class GithubSource extends Activity {
    ...
    async listIssues({ user, repo, owner, since }, gh) {
        ...
        const issues = await getForRepo({ owner, repo, since });
    ...
    async listPullRequests({ repo, owner }, gh) {
        ...
        const requests = await getAll({ owner, repo });
    ...
    async listMergedPullRequests({ user, repo, owner }, gh) {
        let mergedPRs = [];
        const requests = await gh.pullRequests.getAll({ user, repo, owner });
