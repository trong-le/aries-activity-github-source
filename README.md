...
### Repo
The repo is the specific repository to grab information from. Required in all methods but `listOrgRepos`. Note: setting the repo field to `all` will cause the activity to first fetch a list of all repositories that the user account has access to, then run the specified `method` on each repository. From Github: "This includes repositories owned by the authenticated user, repositories where the authenticated user is a collaborator, and repositories that the authenticated user has access to through an organization membership."
```javascript
"repo": "test_repo"
```
